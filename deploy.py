...
# nginx can reach us over Docker DNS at http://eka-webhook-app:80. Otherwise fall
...
def step_finalize(state):
    ...
    print(f"  View logs:    {PROG} status   (or) docker compose --env-file {state.config_file} "
          f"-f {state.compose_file} {profile_flag}logs -f eka-webhook-app")
...
def cmd_upgrade(state):
    ...
    if compose(state, ["up", "-d", "eka-webhook-app"], check=False).returncode != 0:
